 ...
 * values into errno plus -1, matching the POSIX contract musl already has.
 */
#include <lib/syscall.h>

/* File / I/O */
long write(int fd, const void *buf, size_t n) {
    return syscall_result(syscall3(SYS_WRITE, fd, (sysarg_t)(uintptr_t)buf,
                                  (sysarg_t)(uintptr_t)n));
}
long read(int fd, void *buf, size_t n) {
    return syscall_result(syscall3(SYS_READ, fd, (sysarg_t)(uintptr_t)buf,
                                  (sysarg_t)(uintptr_t)n));
}
long open(const char *path, int flags) {
    return syscall_result(syscall2(SYS_OPEN, (sysarg_t)(uintptr_t)path, flags));
}
long close(int fd) {
    return syscall_result(syscall1(SYS_CLOSE, fd));
}
long lseek(int fd, long off, int whence) {
    return syscall_result(syscall3(SYS_LSEEK, fd, off, whence));
}
long chdir(const char *path) {
    return syscall_result(syscall1(SYS_CHDIR, (sysarg_t)(uintptr_t)path));
}
...
int mount(const char *source, const char *target, const char *filesystemtype,
    ...
    return (int)syscall_result(syscall5(SYS_MOUNT, (sysarg_t)(uintptr_t)source,
                                        ...
                                        (sysarg_t)mountflags,
                                        (sysarg_t)(uintptr_t)data));
}
int umount2(const char *target, int flags) {
    return (int)syscall_result(syscall2(SYS_UMOUNT, (sysarg_t)(uintptr_t)target,
                                        (sysarg_t)flags));
}
int umount(const char *target) { return umount2(target, 0); }
char *getcwd(char *buf, size_t size) {
    long rc = syscall_result(
        syscall2(SYS_GETCWD, (sysarg_t)(uintptr_t)buf, (sysarg_t)size));
    return rc < 0 ? 0 : buf;
}
/* Memory
 *
...
void *mmap(void *addr, size_t len, int prot, int flags) {
    long rc = syscall_result(
        syscall4(SYS_MMAP, (sysarg_t)(uintptr_t)addr, (sysarg_t)len, prot, flags));
    return rc < 0 ? MAP_FAILED : (void *)(uintptr_t)rc;
}
int mprotect(void *addr, size_t len, int prot) {
    return (int)syscall_result(
        syscall3(SYS_MPROTECT, (sysarg_t)(uintptr_t)addr, (sysarg_t)len, prot));
}
int munmap(void *addr, size_t len) {
    return (int)syscall_result(
        syscall2(SYS_MUNMAP, (sysarg_t)(uintptr_t)addr, (sysarg_t)len));
}
long readdir(unsigned *index, char *buf, size_t n) {
    return syscall_result(syscall3(SYS_READDIR_INDEX, (sysarg_t)(uintptr_t)index,
                                   (sysarg_t)(uintptr_t)buf, (sysarg_t)n));
}
long unlink(const char *path) {
    return syscall_result(syscall1(SYS_UNLINK, (sysarg_t)(uintptr_t)path));
}
/* Process control */
...
long kill(long pid, int signal) {
    return syscall_result(syscall2(SYS_KILL, pid, signal));
}
